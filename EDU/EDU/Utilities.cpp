...
void Rotate(const Pos& inPos, Pos* outPos, float angle)
{	
	float a = cos(angle);
	float b = -sin(angle);
	float c = sin(angle);
...
void CollisionMove(Pos* pos1, Pos* pos2, hgeRect* r1, hgeRect* r2, float d)
{
	hgeVector v(pos1->x-pos2->x, pos1->y-pos2->y);
	if(r1->Intersect(r2))
	{
		v.Normalize();
		pos1->x += v.x*d;
